...
- **Açık risk (kabul edilerek not düşüldü, bu PR'da çözülmedi):** `TicketSlotManager`'ın lookahead kuyruğu (`UpcomingQueueSize`, varsayılan 10) hâlâ ileri bilet üretmeye çalışıyor — bir Day'in `ticketSequence.Count < 10` olması durumunda kuyruk day başlar başlamaz `DayTicketSequenceProvider`'ı tüketip `NextTicket()`'ın throw etmesine yol açar. Bu, PR-6.6'nın (`DayValidator`) yakalaması gereken bir authoring kısıtı — Day'ler en az `UpcomingQueueSize` kadar bilet içermeli.
