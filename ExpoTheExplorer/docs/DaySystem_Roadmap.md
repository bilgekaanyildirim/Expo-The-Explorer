...
9. **PR-3 regresyonu — day tamamlanınca board üretimi kalıcı olarak duruyor (henüz düzeltilmedi, bilinçli olarak sonraya bırakıldı).** `GameManager.OnTicketAssigned`, `boardDistributor.OnOrderPlaced(...)`'in **tek çağrı noktası** — `State.TicketAssigned` event'ine bağlı. `TicketSlotManager.IsDayComplete=true` olduğunda (PR-3'ün `AssignTicket` guard'ı) bu event'in tek publisher'ı olan `AssignTicket` erken `return` ediyor, yani `TicketAssigned` bir daha hiç ateşlenmiyor. Sonuç: day'in teslimat hedefine ulaşıldığı anda board üretimi tamamen ve kalıcı olarak duruyor — o anda hâlâ aktif kalan biletlerin ihtiyaç duyduğu item'lar da bir daha asla board'a düşmüyor. **Hazır fix taslağı:** `GameManager`'a `RunBoardDistribution()` adında küçük bir yardımcı (`boardDistributor.OnOrderPlaced(State.TicketSlots, TicketSlotManager.UpcomingTickets)`'i sarmalayan) eklenip `OnTicketAssigned`'da kullanılır; `OnDayCompleted`'e `TicketSlotManager.PauseForDayComplete()`'ten hemen sonra bu yardımcının **bir kez daha** çağrılması eklenir — üretim tamamen durmadan önce, o an aktif kalan biletlerin ihtiyaçları son kez garantiye bağlanır. `RetryDay()`/`AdvanceToNextDay()` etkilenmiyor (zaten `ResetSlotsForNewDay()` üzerinden `TicketAssigned`'i normal şekilde tekrar tetikliyorlar).
