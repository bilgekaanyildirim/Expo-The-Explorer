...
- **KEYS gate playing, and they are a different resource from lives** (`decisions.md` D-065, plan in `.claude/key-plan.md`). A key is permission to play a day at all: **cap 5, +1 every 30 real minutes** (the clock runs while the game is closed), **spent only when LEAVING a lost day** — Retry or Main Menu (D-068) — so a paid Continue costs none, `RetryCompletedDay` costs none (that day was won, not lost), and *starting* a day is a gate rather than a price. The charge is gated on `GameState.IsAwaitingContinue` rather than on which method ran, which is what makes the debug R-key retry free when the day was going fine.
- **No button is ever disabled for want of a key.** The check happens **at click time** and opens the out-of-keys popup instead (`NoKeysPopupView`, D-069): a greyed-out button does not tell the player why, and the popup can offer both ways forward (wait for the timer, or 40 Gems for a full refill). Two buttons ask it: Play on the main screen and Retry on the Game Over popup. **Main Menu is never gated at all** — blocking the only exit from a lost day at zero keys is a softlock, so its spend floors at zero and the player still leaves.
...
- 3 fixed powerups: (1) Auto-Collect — auto-places required-pool items into the correct trays, (2) Time Reset — refreshes active ticket timers, (3) Noise Clear — **REMOVES from the board every item no active ticket needs.**
- **Noise Clear is not a visual effect and has no duration** (user's correction, 2026-08-25 — an earlier design, and an earlier version of this file, said it faded noise for a few seconds). It mutates the board: instant, permanent, and it frees cells. That reverses its standing among the three — it is now plausibly the STRONGEST, so its authored price and starting stock need re-tuning. The rule is by IDENTITY, not by count: surplus copies of a food a ticket does need are kept, because a player calls a third burger "too many", not "not needed". Emptied cells immediately backfill from `BoardGrid`'s pending-spawn queue, so clearing also lets required items that could not fit finally land.
...
- Earned two ways: **completing a day** (per-type authored amount) and **Gem purchase** through `Wallet`, which stays the single writer of Gems. (The original design granted them on level-up and via "events"; there is no level system and no event system, so day completion — the candidate the GDD itself named — took that slot.)
- **Buying happens on the MAIN SCREEN only; the day scene only spends.** The powerup shop is its own panel with its own button, following `MetaShopView`'s shape but deliberately not living inside it (that file owns props and SoftMoney and belongs to MetaSystem). Do not add a buy button to the day HUD: opening a store mid-day suspends the time pressure the powerup exists to relieve. **Pressing a powerup with no charges does nothing at all** — no popup, unlike `NoKeysPopupView`, because an empty key locks the player out of playing while an empty charge only costs them a convenience.
...
- Every number (starting stock, Gem cost, per-day grant, clarity duration and dim amount) is authored on `PowerupConfig`. **No cooldown or per-day use cap** — the stock is the limit.
