...
- **The wallet, the day index, the keys and the owned meta props persist between sessions; nothing else does.** SoftMoney, Gems, `CurrentDayIndex`, `Keys`/`LastKeyRegenUtcTicks` (D-066) and `OwnedMetaItemIds` are saved to `player_profile.json` (`Application.persistentDataPath`) and restored on launch, so the main screen's Play opens the Day the player left off on (`decisions.md` D-012, which closes `docs/DaySystem_Roadmap.md` Q4). A saved index is **clamped** against the authored Day catalog on load — an index past the last Day would otherwise leave `CurrentDay` null and throw on the first ticket.
