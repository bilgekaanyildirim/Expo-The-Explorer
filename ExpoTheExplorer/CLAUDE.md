...
- **The wallet persists between sessions; nothing else does.** SoftMoney and Gems are saved to `player_profile.json` (`Application.persistentDataPath`) and restored on launch. Lives still start full and the player still starts at Day 0 every run — persisting `CurrentDayIndex` is a separate, still-open decision (`docs/DaySystem_Roadmap.md` Q4).
- **Money becomes permanent only when a day is completed.** The save happens on `GameState.DayCompleted`, so quitting mid-day loses that day's earnings, and a failed day never writes at all — the same "a day attempt is atomic" contract the in-memory revert enforces. The one extra write is `RetryCompletedDay`, which has to correct a figure a completed day already banked.
- **The save file carries a schema version** (`PlayerProfileStore.CurrentVersion`), stamped on every write, as the root CLAUDE.md invariant requires. Loading accepts versions 1 up to this build's and refuses both an unversioned file (version 0 — its numbers could mean anything, and it is indistinguishable from a real 0 balance) and one from a newer build. Adding a profile field means bumping the version; an older file simply lacks it and reads it as 0, so check that 0 is a safe default for whatever you add.
